...
void sboxfunction(int sr[4], int xorArray[8]) {
    int sbox[2][2] = {{0, 1}, {2, 3}};
    int s0[4][4] = {{1, 0, 3, 2}, {3, 2, 1, 0}, {0, 2, 1, 3}, {3, 1, 3, 2}};
    int s1[4][4] = {{0, 1, 2, 3}, {2, 0, 1, 3}, {3, 0, 1, 0}, {2, 1, 0, 3}};
    int sresult[2];
    i = 0;
    sresult[0] = s0[sbox[xorArray[0]][xorArray[3]]][sbox[xorArray[1]][xorArray[2]]];
    sresult[1] = s1[sbox[xorArray[4]][xorArray[7]]][sbox[xorArray[5]][xorArray[6]]];
    for (j = 0; j < 2; j++) {
        switch (sresult[j]) {
            case 0:
                sr[i++] = 0;
...
void cryption(int pt[8], int k1[8], int k2[8]) {
    ...
    int aip[8], aep[8], xorArray[8], as[8], as1[8];
    int p4[4] = {2, 4, 3, 1};
    // intial permutation
    for (i = 0; i < 8; i++) {
        aip[i] = pt[(ip[i] - 1)];
    ...
    // xorArray of key1 (for encryption) / key2 (for decryption) and expansion permutation
    for (i = 0; i < 8; i++) {
        xorArray[i] = aep[i] ^ k1[i];
    }

    sboxfunction(sr, xorArray);  // finding sbox values
    ...
    // xorArray of key2 (for encryption) / key1 (for decryption) and expansion permutation
    for (i = 0; i < 8; i++) {
        xorArray[i] = aep[i] ^ k2[i];
    }

    sboxfunction(sr, xorArray);
